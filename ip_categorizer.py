...
def load_network_ranges(csv_file):
    ...
    try:
        ...
        # Look for category column (containing msft/zx)
        for col in df.columns:
            if any(df[col].astype(str).str.contains('msft|zx', case=False, na=False)):
                category_col = col
                break
...
def categorize_ips(ip_addresses, networks):
    ...
    """
    results = []
    categorized_count = {'msft': 0, 'zx': 0, 'uncategorized': 0}
    ...
    print(f"MSFT: {categorized_count['msft']}")
    print(f"ZX: {categorized_count['zx']}")
    print(f"Uncategorized: {categorized_count['uncategorized']}")
